...
EstacionFacade <- R6Class("EstacionFacade",
	...
	public = list(
      ...
      if (! is.null(institucion_siglas)) {
		...
		insertar = function(datos.estacion) {
		  # a. Iniciar transaccion
		  DBI::dbBegin(private$con)
		  
		  tryCatch({
		    # b. Bloquear tablas
		    DBI::dbSendQuery(private$con, "LOCK TABLE estacion IN EXCLUSIVE MODE")
		    
		    # c. Insertar datos de estaciones y tipos
		    DBI::dbWriteTable(private$con, "estacion", datos.estacion, append = TRUE, row.names = FALSE)
